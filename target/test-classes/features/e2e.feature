Feature: End 2 end - create, update, delete the user, verify the user deleted successfully
  ...
  @E2E
  Scenario: Verify the end 2 end flow for creating, updating, deleting the user
    ...
    * print "POST response ID :: "+responseUserID
    # PUT call - Modifying the name of the user
    * def putRequestBody = {"status": "inactive"}
    * putRequestBody.name = randomPrefix + " update name"
    * print "-- PUT request -- ", putRequestBody
    And path '/public/v2/users/' + responseUserID
    And header Authorization = 'Bearer ' + myToken
    And header Content-Type = "application/json"
    ...
    Then status 200
    * print response
    * def putResponseBody = response
    * print 'PUT Response Payload:', response
    * def putResponseBody = response
    * match putResponseBody.status == 'inactive'
    * match putResponseBody.name == putRequestBody.name
    ...
    * match responseBody.name == putResponseBody.name
